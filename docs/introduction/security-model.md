...
| **Task Runner** | Run existing tasks. |
...
with that project's credentials. A Task Runner can start only existing templates
...
does not disclose them. APIs do not return persisted credential values; the UI shows
that a secret is set, not what it is. The authenticated, reauthenticated TOTP
enrollment ceremony is a narrow exception: it returns its owner's one-time provisioning
URI and recovery codes, which must not be logged.
...
- The **Docker** or **Kubernetes** executor gives each job a fresh container or Pod
  ...
  host exposure, but the runner host, Docker daemon or Kubernetes cluster, image
  allow-list, mounts, and network policy remain part of the operator's trust boundary.

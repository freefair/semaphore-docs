...
Project runner routes apply the same boundary with project-scoped permissions. They add the typed actions `project_runner_list`, `project_runner_read`, `project_runner_create`, `project_runner_registration_issue`, `project_runner_update`, `project_runner_set_active`, `project_runner_cache_clear`, and `project_runner_delete`, plus the `project_runner` target type. Assignment conflicts use the allowlisted `active_assignments` reason. Target IDs are restricted to `project:<positive integer>` and `runner:<positive integer>`; names, tags, registration material, task payloads, and request bodies cannot enter the audit record. Each project-runner event carries the requested positive `project_id`, and a `project:<id>` target must match it. Cross-project lookups return `404` and record a denied result under the requested project without disclosing the runner's origin project.

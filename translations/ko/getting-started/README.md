...
안내에 따라 설정하려면 공식 [Semaphore 설치 페이지](../../../docs/admin-guide/configuration/config-file.md)에서 릴리스를 선택하고 설정을 생성하여 해당 다운로드 또는 실행 명령을 확인하세요.
...
| 설치 방법 | 적합한 환경 | 상세 가이드 |
| --- | --- | --- |
| **네이티브 패키지** | 지원되는 Linux 서버 | [패키지 관리자로 설치](../../../docs/admin-guide/installation/package-manager.md) |
| **Docker Compose** | 빠른 격리 환경 구성 또는 컨테이너 호스트 | [Docker 설치](../../../docs/admin-guide/installation/docker.md) |
| **바이너리 압축 파일** | macOS, Windows, FreeBSD 또는 적합한 패키지가 없는 Linux | [바이너리 설치](../../../docs/admin-guide/installation/binary-file.md) |
| **Helm으로 Kubernetes에 설치** | 기존 Kubernetes 클러스터 | [Kubernetes 설치](../../../docs/admin-guide/installation/k8s.md) |
...
> **프로덕션 설치**
>
> Semaphore를 프로덕션에서 사용하기 전에 [설정](../../../docs/admin-guide/configuration.md), [보안](../../../docs/admin-guide/security.md), [러너](../../../docs/admin-guide/runners.md), [고가용성](../../../docs/admin-guide/ha.md), [업그레이드](../../../docs/admin-guide/upgrading.md)를 검토하세요.
...
![Semaphore 로그인 화면](../../../static/assets/getting-started/sign-in.jpg)

초기 설정에는 프로젝트와 사용자를 생성할 수 있는 관리자 계정을 사용합니다. 일반 사용자는 관리자가 계정을 생성하고 프로젝트 접근 권한을 부여한 후 같은 페이지에서 로그인합니다. [사용자 관리](../../../docs/user-guide/admin/users.md)를 참조하세요.
...
![사용 가능한 모든 필드가 표시된 빈 New Project 양식](../../../static/assets/getting-started/new-project-empty.jpg)

새 프로젝트에는 **Task Templates**, **Workflows**, **Schedule**, **Inventory**, **Variable Groups**, **Key Store**, **Repositories** 섹션이 있습니다. 프로젝트 설정, 팀 접근 권한, 활동, 기록은 [프로젝트](../../../docs/user-guide/projects.md)를 참조하세요.

<details>
<summary>이 단계 보기</summary>

![빈 Semaphore 인스턴스에서 첫 프로젝트 생성](../../../static/assets/getting-started/create-first-project.gif)
...
![리소스와 작업을 추가하기 전의 빈 Semaphore 프로젝트 화면](../../../static/assets/getting-started/after-sign-in.jpg)
...
다음 다이어그램은 이러한 리소스가 실행으로 이어지는 과정을 보여 줍니다.
  ...
  ![Semaphore 리소스와 트리거가 작업 실행으로 이어지는 과정](../../../static/assets/getting-started/core-concepts.svg)
...
| 개념 | 역할 |
| --- | --- |
| [**Project**](../../../docs/user-guide/projects.md) | 자동화 리소스, 권한, 실행 기록을 포함하는 격리된 작업 공간입니다. |
| [**Repository**](../../../docs/user-guide/repositories.md) | 작업에서 사용하는 자동화 파일이 있는 Git 브랜치 또는 태그를 가리킵니다. |
| [**Key Store**](../../../docs/user-guide/key-store.md) | 재사용 가능한 SSH 키, 로그인 자격 증명, 토큰, Ansible Vault 암호를 Git 및 작업 입력과 별도로 저장합니다. |
| [**Inventory**](../../../docs/user-guide/inventory.md) | 관리할 호스트와 그룹, 사용할 자격 증명을 Ansible에 알려 줍니다. |
| [**Variable Group**](../../../docs/user-guide/environment.md) | 하나 이상의 템플릿에서 재사용할 Ansible 변수, 환경 변수, 시크릿을 저장합니다. |
| [**Task Template**](../../../docs/user-guide/task-templates/README.md) | 자동화 유형, 파일, 저장소, 인벤토리, 변수, 프롬프트, 실행 옵션 등 실행할 내용을 저장합니다. |
| [**Task (task run)**](../../../docs/user-guide/tasks.md) | 고유한 입력, 상태, 타임스탬프, 로그, 상세 정보, 결과를 가진 한 번의 실행입니다. |
| **Workflow** | 성공, 실패, 승인, 메모 분기가 있는 여러 단계의 경로로 작업 템플릿을 연결합니다. |
| [**Schedule**](../../../docs/user-guide/schedules.md) | cron 표현식에 따라 작업 템플릿이나 워크플로를 한 번 또는 반복해서 시작합니다. |
| [**Runner**](../../../docs/admin-guide/runners.md) | 다른 네트워크나 보안 영역 등 Semaphore 주 서버 외부에서 대기 중인 작업을 실행합니다. |

## 5. 저장소 연결하기

Repository는 Semaphore를 Git에 저장된 자동화 코드와 연결합니다. Semaphore 자체에는 플레이북이 저장되지 않습니다. 자신의 저장소를 연결하거나 아래 공개 데모 값을 사용해 예제를 그대로 재현하세요. [통합](../../../docs/user-guide/integrations.md)은 GitHub, GitLab 또는 다른 웹훅 소스에서 자동화를 시작하는 별도 기능입니다.
...
![Semaphore 공개 데모 저장소 정보가 입력된 저장소 양식](../../../static/assets/getting-started/repository-settings.jpg)
...
![프로젝트 저장소 목록에 연결된 Demo 저장소](../../../static/assets/getting-started/connected-repository.jpg)

비공개 저장소에는 `None` 대신 적절한 Key Store 자격 증명을 선택합니다. 로컬 경로, HTTPS, SSH, 브랜치, 자격 증명, 의존성 파일은 [저장소](../../../docs/user-guide/repositories.md)를 참조하세요.
...
![관리 대상 호스트에서 사용하는 계정의 New SSH Key 양식](../../../static/assets/getting-started/add-managed-host-ssh-key.jpg)

스크린샷에는 실제로 작동하는 시크릿이 아닌 자리 표시자가 들어 있습니다. 개인 키를 문서, 스크린샷, 작업 인수 또는 버전 관리에 공개하지 마세요.

Semaphore는 시크릿을 로컬에 저장하거나 [HashiCorp Vault](../../../docs/user-guide/key-store/hashicorp-vault.md), [Devolutions Server](../../../docs/user-guide/key-store/devolutions-server.md) 같은 외부 시크릿 저장소와 통합할 수 있습니다. 지원되는 모든 자격 증명 유형과 저장 옵션은 [키 저장소](../../../docs/user-guide/key-store.md)를 참조하세요.
...
![데모 저장소 값으로 설정한 Ansible 파일 인벤토리](../../../static/assets/getting-started/ansible-inventory-settings.jpg)
...
정적, 파일 기반, 동적 인벤토리는 [인벤토리](../../../docs/user-guide/inventory.md)를 참조하세요.
...
![표 편집기에서 설정한 변수 그룹](../../../static/assets/getting-started/variable-group-table.jpg)

우선순위 규칙과 시크릿 저장 옵션은 [변수 그룹](../../../docs/user-guide/environment.md)을 참조하세요.
...
데모는 Git 서브모듈에서 해당 역할을 다운로드하고 the demo's configured public target에 ICMP 요청을 한 번 보내므로 실행 호스트에 GitHub 접근과 아웃바운드 ICMP가 필요합니다. ICMP가 차단되어 있다면 로컬 `get-started.yml` 예제를 사용하세요.

![연결한 GitHub 저장소의 ping.yml](../../../static/assets/getting-started/demo-playbook-github.jpg)
...
![저장소, 인벤토리, 변수 그룹을 설정한 Ansible 작업 템플릿](../../../static/assets/getting-started/ansible-task-template-settings.jpg)

<details>
<summary>이 단계 보기</summary>

![Ansible 활성화 및 첫 Ansible 작업 템플릿 생성](../../../static/assets/getting-started/create-ansible-template.gif)
...
전체 필드와 실행 옵션은 [Ansible 템플릿](../../../docs/user-guide/apps/ansible.md) 및 [작업 템플릿](../../../docs/user-guide/task-templates/README.md)을 참조하세요.
...
![추가 옵션을 설정하지 않은 Ansible 플레이북의 New Task 대화 상자](../../../static/assets/getting-started/run-ansible-task-clean.jpg)
...
![ping 출력과 PLAY RECAP이 포함된 성공한 Ansible 작업 로그](../../../static/assets/getting-started/ansible-task-log-variable-group.jpg)
...
![템플릿, 커밋, 시간 정보를 보여 주는 작업 상세 정보](../../../static/assets/getting-started/ansible-task-details.jpg)

![OK 및 Not OK 호스트 수를 보여 주는 작업 요약](../../../static/assets/getting-started/ansible-task-summary.jpg)
...
![Ansible 작업 실행 후 로그와 상세 정보 확인](../../../static/assets/getting-started/run-and-inspect-task.gif)

</details>

### 이전 실행 찾기

작업 창을 닫으면 템플릿의 **Tasks** 탭으로 돌아갑니다. 각 실행에는 고유한 작업 번호, 상태, 사용자, 시작 시각, 소요 시간, 보관된 로그가 있습니다. **Dashboard → History**는 프로젝트 내 모든 템플릿의 실행을 보여 줍니다. 자세한 내용은 [작업](../../../docs/user-guide/tasks.md) 및 [프로젝트 기록](../../../docs/user-guide/projects/history.md)을 참조하세요.

![성공한 작업 실행이 있는 Ansible 템플릿 기록](../../../static/assets/getting-started/ansible-template-history.jpg)
...
![예제 작업을 매일 03:00에 실행하도록 설정한 cron 일정](../../../static/assets/getting-started/create-cron-schedule.jpg)

Semaphore는 저장 전에 설정된 시간대를 표시하고 다음 실행 시각을 계산합니다. 예약 실행은 템플릿과 같은 저장소, 인벤토리, 변수 그룹, 실행 설정을 사용합니다. 템플릿에 프롬프트가 있다면 일정에서 해당 값을 제공할 수 있습니다. cron 문법, 시간대 설정, 일회성 실행, 예약 매개변수는 [일정](../../../docs/user-guide/schedules.md)을 참조하세요.
...
- 저장소에 인증이 필요하다면 [키 저장소](../../../docs/user-guide/key-store.md)에 적절한 비공개 자격 증명을 추가합니다.
- 여러 템플릿에 성공, 실패, 승인, 메모의 순서 있는 경로가 필요하다면 **Workflow**를 만듭니다.
- GitHub, GitLab 또는 다른 시스템에서 인증된 웹훅 트리거를 사용하려면 [통합](../../../docs/user-guide/integrations.md)을 사용합니다.
- 리소스 관리와 템플릿 실행을 프로그램에서 수행하려면 [API](../../../docs/reference/api.md)를 사용합니다.
- 다른 네트워크, 운영 체제 또는 보안 영역에서 실행해야 한다면 [원격 러너](../../../docs/admin-guide/runners.md)를 추가합니다.

프로덕션에서는 Semaphore를 HTTPS로 제공하고, 데이터베이스와 액세스 키 암호화 시크릿을 함께 백업하며, 중앙 인증을 설정하고 [보안](../../../docs/admin-guide/security.md), [로그](../../../docs/admin-guide/logs.md), [업그레이드](../../../docs/admin-guide/upgrading.md)를 검토하세요.

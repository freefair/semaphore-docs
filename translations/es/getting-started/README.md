...
Para una configuración guiada, use la [página oficial de instalación de Semaphore](../../../docs/admin-guide/configuration/config-file.md) para seleccionar la versión, generar la configuración y obtener los comandos de descarga o ejecución correspondientes.
...
| --- | --- | --- |
| **Paquete nativo** | Un servidor Linux compatible | [Instalación mediante gestor de paquetes](../../../docs/admin-guide/installation/package-manager.md) |
| **Docker Compose** | Una configuración aislada rápida o un host de contenedores | [Instalación con Docker](../../../docs/admin-guide/installation/docker.md) |
| **Archivo binario** | macOS, Windows, FreeBSD o Linux sin un paquete adecuado | [Instalación del binario](../../../docs/admin-guide/installation/binary-file.md) |
| **Kubernetes con Helm** | Un clúster Kubernetes existente | [Instalación en Kubernetes](../../../docs/admin-guide/installation/k8s.md) |
...
>
> Antes de usar Semaphore en producción, revise [Configuración](../../../docs/admin-guide/configuration.md), [Seguridad](../../../docs/admin-guide/security.md), [Runners](../../../docs/admin-guide/runners.md), [Alta disponibilidad](../../../docs/admin-guide/ha.md) y [Actualización](../../../docs/admin-guide/upgrading.md).
...
![Pantalla de inicio de sesión de Semaphore](../../../static/assets/getting-started/sign-in.jpg)

Use la cuenta de administrador para la configuración inicial porque puede crear proyectos y usuarios. Los usuarios normales acceden desde la misma página cuando un administrador crea sus cuentas y les concede acceso al proyecto. Consulte [Gestión de usuarios](../../../docs/user-guide/admin/users.md).
...
![Formulario New Project vacío con todos los campos disponibles](../../../static/assets/getting-started/new-project-empty.jpg)

El nuevo proyecto contiene las secciones **Task Templates**, **Workflows**, **Schedule**, **Inventory**, **Variable Groups**, **Key Store** y **Repositories**. Consulte [Proyectos](../../../docs/user-guide/projects.md) para conocer la configuración del proyecto, el acceso del equipo, la actividad y el historial.

<details>
<summary>Vea este paso</summary>

![Creación del primer proyecto en una instancia vacía de Semaphore](../../../static/assets/getting-started/create-first-project.gif)
...
![Interfaz de un proyecto vacío de Semaphore antes de añadir recursos y tareas](../../../static/assets/getting-started/after-sign-in.jpg)
...
El diagrama muestra cómo estos recursos producen una ejecución:
  ...
  ![Cómo los recursos y disparadores de Semaphore generan una ejecución de tarea](../../../static/assets/getting-started/core-concepts.svg)
...
| Concepto | Función |
| --- | --- |
| [**Project**](../../../docs/user-guide/projects.md) | Un espacio de trabajo aislado con recursos de automatización, permisos e historial de ejecuciones. |
| [**Repository**](../../../docs/user-guide/repositories.md) | Apunta a la rama o etiqueta Git que contiene los archivos de automatización de una tarea. |
| [**Key Store**](../../../docs/user-guide/key-store.md) | Almacena claves SSH, credenciales, tokens y contraseñas de Ansible Vault reutilizables fuera de Git y de las entradas de tareas. |
| [**Inventory**](../../../docs/user-guide/inventory.md) | Indica a Ansible qué hosts y grupos debe administrar y qué credenciales usar. |
| [**Variable Group**](../../../docs/user-guide/environment.md) | Almacena variables de Ansible, variables de entorno y secretos reutilizables para una o varias plantillas. |
| [**Task Template**](../../../docs/user-guide/task-templates/README.md) | Guarda qué ejecutar: tipo de automatización, archivo, repositorio, inventario, variables, parámetros solicitados y opciones de ejecución. |
| [**Task (task run)**](../../../docs/user-guide/tasks.md) | Una ejecución con sus propias entradas, estado, marcas de tiempo, registro, detalles y resultado. |
| **Workflow** | Conecta plantillas en una secuencia de varios pasos con ramas de éxito, fallo, aprobación y notas. |
| [**Schedule**](../../../docs/user-guide/schedules.md) | Inicia una plantilla de tarea o un flujo una vez o repetidamente mediante una expresión cron. |
| [**Runner**](../../../docs/admin-guide/runners.md) | Ejecuta tareas en cola fuera del servidor principal de Semaphore, por ejemplo en otra red o zona de seguridad. |

## 5. Conecte el repositorio

Un Repository conecta Semaphore con la automatización almacenada en Git; Semaphore no almacena el playbook en sí. Conecte su repositorio o use los valores de la demostración pública siguientes para reproducir exactamente el ejemplo. Las [Integraciones](../../../docs/user-guide/integrations.md) son una función independiente para iniciar automatizaciones desde GitHub, GitLab u otras fuentes de webhooks.
...
![Formulario de repositorio con el repositorio público de demostración de Semaphore](../../../static/assets/getting-started/repository-settings.jpg)
...
![Repositorio Demo conectado en la lista de repositorios del proyecto](../../../static/assets/getting-started/connected-repository.jpg)

Para un repositorio privado, seleccione credenciales adecuadas del Key Store en lugar de `None`. Consulte [Repositorios](../../../docs/user-guide/repositories.md) para rutas locales, HTTPS, SSH, ramas, credenciales y archivos de dependencias.
...
![Formulario New SSH Key para la cuenta utilizada en los hosts administrados](../../../static/assets/getting-started/add-managed-host-ssh-key.jpg)
...
Semaphore puede almacenar secretos localmente o integrar almacenes externos como [HashiCorp Vault](../../../docs/user-guide/key-store/hashicorp-vault.md) y [Devolutions Server](../../../docs/user-guide/key-store/devolutions-server.md). Consulte [Almacén de claves](../../../docs/user-guide/key-store.md) para todos los tipos de credenciales y opciones de almacenamiento compatibles.
...
![Inventario de archivo de Ansible configurado con los valores del repositorio de demostración](../../../static/assets/getting-started/ansible-inventory-settings.jpg)
...
Consulte [Inventario](../../../docs/user-guide/inventory.md) para inventarios estáticos, basados en archivos y dinámicos.
...
![Grupo de variables configurado en el editor de tablas](../../../static/assets/getting-started/variable-group-table.jpg)

Consulte [Grupos de variables](../../../docs/user-guide/environment.md) para reglas de precedencia y opciones de almacenamiento de secretos.
...
La demostración descarga ese rol desde un submódulo Git y envía una solicitud ICMP a the demo's configured public target, por lo que el host de ejecución necesita acceso a GitHub e ICMP saliente. Si ICMP está bloqueado, use el ejemplo local `get-started.yml`.

![ping.yml en el repositorio GitHub conectado](../../../static/assets/getting-started/demo-playbook-github.jpg)
...
![Plantilla de tarea de Ansible con su repositorio, inventario y grupo de variables](../../../static/assets/getting-started/ansible-task-template-settings.jpg)

<details>
<summary>Vea este paso</summary>

![Activación de Ansible y creación de la primera plantilla de tarea de Ansible](../../../static/assets/getting-started/create-ansible-template.gif)
...
Consulte [Plantillas de Ansible](../../../docs/user-guide/apps/ansible.md) y [Plantillas de tareas](../../../docs/user-guide/task-templates/README.md) para todos los campos y opciones de ejecución.
...
![Diálogo New Task sin opciones adicionales para el playbook de Ansible](../../../static/assets/getting-started/run-ansible-task-clean.jpg)
...
![Registro de tarea de Ansible correcta con salida de ping y PLAY RECAP](../../../static/assets/getting-started/ansible-task-log-variable-group.jpg)
...
![Detalles de la tarea con plantilla, commit e información temporal](../../../static/assets/getting-started/ansible-task-details.jpg)

![Resumen de tarea con recuentos de hosts OK y Not OK](../../../static/assets/getting-started/ansible-task-summary.jpg)
...
![Ejecución de la tarea de Ansible y revisión del registro y los detalles](../../../static/assets/getting-started/run-and-inspect-task.gif)

</details>

### Busque ejecuciones anteriores

Cierre la ventana de tarea para volver a la pestaña **Tasks** de la plantilla. Cada ejecución tiene su número de tarea, estado, usuario, hora de inicio, duración y registro conservado. **Dashboard → History** muestra las ejecuciones de todas las plantillas del proyecto. Consulte [Tareas](../../../docs/user-guide/tasks.md) e [Historial del proyecto](../../../docs/user-guide/projects/history.md) para más detalles.

![Historial de una plantilla de Ansible con ejecuciones correctas](../../../static/assets/getting-started/ansible-template-history.jpg)
...
- Añada las credenciales privadas adecuadas en [Almacén de claves](../../../docs/user-guide/key-store.md) si su repositorio requiere autenticación.
- Cree un **Workflow** cuando varias plantillas necesiten rutas ordenadas de éxito, fallo, aprobación o notas.
- Use [Integraciones](../../../docs/user-guide/integrations.md) para disparadores webhook autenticados desde GitHub, GitLab u otros sistemas.
- Use la [API](../../../docs/reference/api.md) para gestionar recursos e iniciar plantillas mediante programación.
- Añada un [runner remoto](../../../docs/admin-guide/runners.md) si la ejecución debe realizarse en otra red, sistema operativo o zona de seguridad.

Para producción, publique Semaphore mediante HTTPS, haga copias de seguridad conjuntas de la base de datos y el secreto de cifrado de las claves de acceso, configure autenticación centralizada y revise [Seguridad](../../../docs/admin-guide/security.md), [Registros](../../../docs/admin-guide/logs.md) y [Actualización](../../../docs/admin-guide/upgrading.md).

...
- **Docker（额外软件包）**：将 `requirements.txt` 挂载到服务器或运行器容器的 `/etc/semaphore/requirements.txt`。Semaphore 会在每次容器启动时将其安装到内置的 Python 虚拟环境中。参见[安装额外的 Python 依赖](../../../../docs/admin-guide/installation/docker.md#installing-additional-python-dependencies)。

...
以及它周围的环境。在进行[安装](../../../docs/admin-guide/installation.md)之前先过一遍本页，
安装本身只需要几分钟。
...
或者把执行挪到[运行器](../../../docs/admin-guide/runners.md)上，改为按运行器规划规格。
...
[高可用](../../../docs/admin-guide/ha.md)需要 PostgreSQL 或 MySQL 以及 Redis，
...
| 你的身份提供方（如果使用） | [LDAP](../../../docs/admin-guide/authentication/ldap.md) 或 [OpenID Connect](../../../docs/admin-guide/authentication/openid.md) 登录。 |
| 你的通知渠道 | 电子邮件、Telegram、Slack 等等。 |

除非你更改端口，用户通过 `3000` 端口访问 Web 界面。请在任何人登录之前，
在它前面加上 [TLS](../../../docs/admin-guide/reverse-proxy/README.md)：会话和 API 令牌都经由它传输。
...
- Docker 镜像自带 Ansible、Terraform、OpenTofu 以及常用依赖。
  ...
  [安装额外的 Python 依赖](../../../docs/admin-guide/installation/docker.md#installing-additional-python-dependencies)。
...
它们都应放在[密钥库](../../../docs/user-guide/key-store.md)中，而不是仓库里。
...
- [安装](../../../docs/admin-guide/installation.md) —— 选择一种方式并完成安装。
- [配置](../../../docs/admin-guide/configuration.md) —— 选项如何提供以及它们的含义。
- [快速上手](../../../docs/getting-started/README.md) —— 从装好的服务器到第一个任务。

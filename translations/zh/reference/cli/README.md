...
所有命令和参数的完整生成列表见[命令参考](../../../../docs/reference/cli/commands.md)。大多数管理任务都有专门的命令组：

| 命令组 | 用途 |
|---------------|---------|
| [`semaphore users`](../../../../docs/reference/cli/users.md) | 添加、修改、删除和查看用户；管理 API 令牌和 TOTP（双因素认证）。 |
| [`semaphore projects`](../../../../docs/reference/cli/projects.md) | 导出和导入项目（备份）。 |
| [`semaphore vaults`](../../../../docs/reference/cli/vaults.md) | 重新加密已存储的密钥并查看加密密钥的使用情况。 |
| [`semaphore runner`](../../../../docs/reference/cli/runners.md) | 以运行器（Runner）模式运行，以及注册/注销运行器。 |
| [`semaphore migrate`](../../../../docs/reference/cli/migrations.md) | 应用或回滚数据库迁移。 |
...
（`setup`、`register`、`start`、`unregister`）请参阅[运行器](../../../../docs/reference/cli/runners.md)。
...
请参阅[数据库迁移](../../../../docs/reference/cli/migrations.md)。

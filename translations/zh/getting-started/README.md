...
如需引导式配置，请使用官方 [Semaphore 安装页面](../../../docs/admin-guide/configuration/config-file.md) 选择版本、生成配置并获取相应的下载或运行命令。
...
| 安装方式 | 适用场景 | 详细指南 |
| --- | --- | --- |
| **原生软件包** | 支持的 Linux 服务器 | [通过包管理器安装](../../../docs/admin-guide/installation/package-manager.md) |
| **Docker Compose** | 快速搭建隔离环境或在容器主机上运行 | [Docker 安装](../../../docs/admin-guide/installation/docker.md) |
| **二进制压缩包** | macOS、Windows、FreeBSD，或没有合适软件包的 Linux | [二进制安装](../../../docs/admin-guide/installation/binary-file.md) |
| **通过 Helm 安装到 Kubernetes** | 已有 Kubernetes 集群 | [Kubernetes 安装](../../../docs/admin-guide/installation/k8s.md) |
...
> **生产环境安装**
>
> 在生产环境使用 Semaphore 前，请阅读[配置](../../../docs/admin-guide/configuration.md)、[安全](../../../docs/admin-guide/security.md)、[Runners](../../../docs/admin-guide/runners.md)、[高可用性](../../../docs/admin-guide/ha.md)和[升级](../../../docs/admin-guide/upgrading.md)。
...
![Semaphore 登录界面](../../../static/assets/getting-started/sign-in.jpg)

首次配置请使用管理员账户，因为它可以创建项目和用户。管理员创建普通用户账户并授予项目访问权限后，普通用户也从同一页面登录。请参阅[用户管理](../../../docs/user-guide/admin/users.md)。
...
![包含所有可用字段的空白 New Project 表单](../../../static/assets/getting-started/new-project-empty.jpg)

新项目包含 **Task Templates**、**Workflows**、**Schedule**、**Inventory**、**Variable Groups**、**Key Store** 和 **Repositories** 等部分。有关项目设置、团队访问、活动和历史，请参阅[项目](../../../docs/user-guide/projects.md)。

<details>
<summary>观看此步骤</summary>

![在空的 Semaphore 实例中创建第一个项目](../../../static/assets/getting-started/create-first-project.gif)
...
![添加资源和任务之前的空白 Semaphore 项目界面](../../../static/assets/getting-started/after-sign-in.jpg)
...
下图展示这些资源如何产生一次运行：
  ...
  ![Semaphore 资源和触发器如何产生任务运行](../../../static/assets/getting-started/core-concepts.svg)
...
| 概念 | 作用 |
| --- | --- |
| [**Project**](../../../docs/user-guide/projects.md) | 包含自动化资源、权限和运行历史的隔离工作空间。 |
| [**Repository**](../../../docs/user-guide/repositories.md) | 指向包含任务所用自动化文件的 Git 分支或标签。 |
| [**Key Store**](../../../docs/user-guide/key-store.md) | 在 Git 和任务输入之外保存可复用的 SSH 密钥、登录凭据、令牌和 Ansible Vault 密码。 |
| [**Inventory**](../../../docs/user-guide/inventory.md) | 告诉 Ansible 要管理哪些主机和组，以及使用哪些凭据。 |
| [**Variable Group**](../../../docs/user-guide/environment.md) | 保存可供一个或多个模板复用的 Ansible 变量、环境变量和机密。 |
| [**Task Template**](../../../docs/user-guide/task-templates/README.md) | 保存运行定义：自动化类型、文件、仓库、inventory、变量、提示参数和执行选项。 |
| [**Task (task run)**](../../../docs/user-guide/tasks.md) | 一次执行，具有独立的输入、状态、时间戳、日志、详情和结果。 |
| **Workflow** | 将任务模板连接成多步骤流程，包含成功、失败、审批和备注分支。 |
| [**Schedule**](../../../docs/user-guide/schedules.md) | 根据 cron 表达式单次或重复启动任务模板或工作流。 |
| [**Runner**](../../../docs/admin-guide/runners.md) | 在 Semaphore 主服务器之外执行排队任务，例如在其他网络或安全区域中。 |

## 5. 连接仓库

Repository 将 Semaphore 连接到 Git 中的自动化代码；Semaphore 本身不存储 playbook。连接您的仓库，或使用以下公共演示配置完整重现示例。[集成](../../../docs/user-guide/integrations.md)是独立功能，用于从 GitHub、GitLab 或其他 webhook 来源启动自动化。
...
![填写了 Semaphore 公共演示仓库信息的仓库表单](../../../static/assets/getting-started/repository-settings.jpg)

现在仓库应出现在列表中。Semaphore 会在任务启动时在执行主机上克隆或更新仓库，而不是在创建 Repository 记录时执行。截图展示了本指南使用的演示配置。

![项目仓库列表中已连接的 Demo 仓库](../../../static/assets/getting-started/connected-repository.jpg)

对于私有仓库，请选择合适的 Key Store 凭据以替代 `None`。有关本地路径、HTTPS、SSH、分支、凭据和依赖文件，请参阅[仓库](../../../docs/user-guide/repositories.md)。
...
![为受管主机所用账户创建密钥的 New SSH Key 表单](../../../static/assets/getting-started/add-managed-host-ssh-key.jpg)

截图中是占位内容，不是可用的机密。切勿在文档、截图、任务参数或版本控制中公开私钥。

Semaphore 可以在本地存储机密，也可以集成 [HashiCorp Vault](../../../docs/user-guide/key-store/hashicorp-vault.md) 和 [Devolutions Server](../../../docs/user-guide/key-store/devolutions-server.md) 等外部机密存储。有关支持的全部凭据类型和存储选项，请参阅[密钥存储](../../../docs/user-guide/key-store.md)。
...
![使用演示仓库配置的 Ansible 文件型 inventory](../../../static/assets/getting-started/ansible-inventory-settings.jpg)
...
有关静态、文件型和动态 inventory，请参阅 [Inventory](../../../docs/user-guide/inventory.md)。
...
![在表格编辑器中配置的变量组](../../../static/assets/getting-started/variable-group-table.jpg)

有关优先级规则和机密存储选项，请参阅[变量组](../../../docs/user-guide/environment.md)。
...
演示从 Git 子模块下载该角色，并向 the demo's configured public target 发送一次 ICMP 请求，因此执行主机需要能访问 GitHub 并允许出站 ICMP。如果 ICMP 被阻止，请改用本地 `get-started.yml` 示例。

![已连接 GitHub 仓库中的 ping.yml](../../../static/assets/getting-started/demo-playbook-github.jpg)
...
![包含仓库、inventory 和变量组的 Ansible 任务模板](../../../static/assets/getting-started/ansible-task-template-settings.jpg)

<details>
<summary>观看此步骤</summary>

![启用 Ansible 并创建第一个 Ansible 任务模板](../../../static/assets/getting-started/create-ansible-template.gif)
...
有关全部字段和执行选项，请参阅 [Ansible 模板](../../../docs/user-guide/apps/ansible.md)和[任务模板](../../../docs/user-guide/task-templates/README.md)。
...
![未启用额外选项的 Ansible playbook New Task 对话框](../../../static/assets/getting-started/run-ansible-task-clean.jpg)
...
![包含 ping 输出和 PLAY RECAP 的成功 Ansible 任务日志](../../../static/assets/getting-started/ansible-task-log-variable-group.jpg)
...
![包含模板、提交和时间信息的任务详情](../../../static/assets/getting-started/ansible-task-details.jpg)

![包含 OK 和 Not OK 主机数量的任务摘要](../../../static/assets/getting-started/ansible-task-summary.jpg)
...
![运行 Ansible 任务并检查日志和详情](../../../static/assets/getting-started/run-and-inspect-task.gif)

</details>

### 查找以前的运行

关闭任务窗口，返回模板的 **Tasks** 选项卡。每次运行都有独立的任务编号、状态、用户、开始时间、耗时和保存的日志。**Dashboard → History** 显示项目中所有模板的运行。更多信息请参阅[任务](../../../docs/user-guide/tasks.md)和[项目历史](../../../docs/user-guide/projects/history.md)。

![包含成功任务运行的 Ansible 模板历史](../../../static/assets/getting-started/ansible-template-history.jpg)
...
![配置为每天 03:00 运行示例任务的 cron 计划](../../../static/assets/getting-started/create-cron-schedule.jpg)

Semaphore 会在保存前显示配置的时区并计算下次运行时间。计划运行使用与模板相同的仓库、inventory、变量组和执行设置。如果模板提供提示参数，计划可以为其提供值。有关 cron 语法、时区配置、单次运行和计划参数，请参阅[计划](../../../docs/user-guide/schedules.md)。
...
- 如果仓库要求身份验证，请在[密钥存储](../../../docs/user-guide/key-store.md)中添加合适的私有凭据。
- 如果多个模板需要有序的成功、失败、审批或备注路径，请构建 **Workflow**。
- 使用[集成](../../../docs/user-guide/integrations.md)接收来自 GitHub、GitLab 或其他系统的经过身份验证的 webhook 触发。
- 使用 [API](../../../docs/reference/api.md) 以编程方式管理资源和启动模板。
- 如果需要在其他网络、操作系统或安全区域执行，请添加[远程 runner](../../../docs/admin-guide/runners.md)。

生产使用时，通过 HTTPS 提供 Semaphore 服务，将数据库与访问密钥加密机密一同备份，配置集中身份验证，并阅读[安全](../../../docs/admin-guide/security.md)、[日志](../../../docs/admin-guide/logs.md)和[升级](../../../docs/admin-guide/upgrading.md)。

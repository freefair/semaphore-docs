...
![スクレイプされた Semaphore のメトリクスを表示する Grafana Explore](../../../static/assets/semaphore-grafana-explore.png)

その後、同じメトリクスを使用してダッシュボードを構築できます。この例では、実行中のタスク数、結果別のタスク総数、goroutine 数、プロセスの常駐メモリという 4 つのパネルで両方のカテゴリをカバーしています。

![Semaphore のパネルを含む Grafana ダッシュボード](../../../static/assets/semaphore-grafana-dashboard.png)

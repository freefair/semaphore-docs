...
ガイドに沿って設定するには、公式の [Semaphore インストールページ](../../../docs/admin-guide/configuration/config-file.md) でリリースを選び、設定を作成して、対応するダウンロードまたは実行コマンドを取得します。
...
| --- | --- | --- |
| **ネイティブパッケージ** | 対応する Linux サーバー | [パッケージマネージャーによるインストール](../../../docs/admin-guide/installation/package-manager.md) |
| **Docker Compose** | 分離した環境の迅速な構築、またはコンテナホスト | [Docker インストール](../../../docs/admin-guide/installation/docker.md) |
| **バイナリアーカイブ** | macOS、Windows、FreeBSD、または適切なパッケージがない Linux | [バイナリのインストール](../../../docs/admin-guide/installation/binary-file.md) |
| **Helm による Kubernetes へのインストール** | 既存の Kubernetes クラスター | [Kubernetes インストール](../../../docs/admin-guide/installation/k8s.md) |
...
> **本番環境へのインストール**
>
> Semaphore を本番で使用する前に、[設定](../../../docs/admin-guide/configuration.md)、[セキュリティ](../../../docs/admin-guide/security.md)、[Runners](../../../docs/admin-guide/runners.md)、[高可用性](../../../docs/admin-guide/ha.md)、[アップグレード](../../../docs/admin-guide/upgrading.md)を確認してください。
...
![Semaphore のサインイン画面](../../../static/assets/getting-started/sign-in.jpg)

初期設定には、プロジェクトとユーザーを作成できる管理者アカウントを使用します。一般ユーザーは、管理者がアカウントを作成してプロジェクトへのアクセスを許可した後、同じページからサインインします。[ユーザー管理](../../../docs/user-guide/admin/users.md)を参照してください。
...
![すべての入力項目を表示した空の New Project フォーム](../../../static/assets/getting-started/new-project-empty.jpg)

新しいプロジェクトには、**Task Templates**、**Workflows**、**Schedule**、**Inventory**、**Variable Groups**、**Key Store**、**Repositories** の各セクションがあります。プロジェクト設定、チームのアクセス、アクティビティ、履歴については[プロジェクト](../../../docs/user-guide/projects.md)を参照してください。

<details>
<summary>この手順を動画で見る</summary>

![空の Semaphore インスタンスに最初のプロジェクトを作成](../../../static/assets/getting-started/create-first-project.gif)
...
![リソースやタスクを追加する前の空の Semaphore プロジェクト画面](../../../static/assets/getting-started/after-sign-in.jpg)
...
次の図は、これらのリソースがどのように実行につながるかを示しています。
  ...
  ![Semaphore のリソースとトリガーがタスク実行につながる仕組み](../../../static/assets/getting-started/core-concepts.svg)
...
| 概念 | 役割 |
| --- | --- |
| [**Project**](../../../docs/user-guide/projects.md) | 自動化リソース、権限、実行履歴を含む分離されたワークスペース。 |
| [**Repository**](../../../docs/user-guide/repositories.md) | タスクが使用する自動化ファイルを含む Git ブランチまたはタグを指定します。 |
| [**Key Store**](../../../docs/user-guide/key-store.md) | 再利用可能な SSH キー、ログイン認証情報、トークン、Ansible Vault パスワードを Git やタスク入力とは別に保存します。 |
| [**Inventory**](../../../docs/user-guide/inventory.md) | 管理するホストとグループ、使用する認証情報を Ansible に伝えます。 |
| [**Variable Group**](../../../docs/user-guide/environment.md) | 1 つ以上のテンプレートで再利用する Ansible 変数、環境変数、シークレットを保存します。 |
| [**Task Template**](../../../docs/user-guide/task-templates/README.md) | 自動化の種類、ファイル、リポジトリ、インベントリ、変数、プロンプト、実行オプションなど、実行内容を保存します。 |
| [**Task (task run)**](../../../docs/user-guide/tasks.md) | 固有の入力、ステータス、タイムスタンプ、ログ、詳細、結果を持つ 1 回の実行。 |
| **Workflow** | 成功、失敗、承認、メモの分岐を持つ複数ステップの経路にタスクテンプレートを接続します。 |
| [**Schedule**](../../../docs/user-guide/schedules.md) | cron 式に従い、タスクテンプレートやワークフローを 1 回または繰り返し起動します。 |
| [**Runner**](../../../docs/admin-guide/runners.md) | 別のネットワークやセキュリティ境界など、Semaphore のメインサーバー外でキュー内のタスクを実行します。 |

## 5. リポジトリを接続する

Repository は Semaphore と Git 内の自動化コードを接続します。Semaphore 自体は playbook を保存しません。自分のリポジトリを接続するか、以下の公開デモの値を使って例をそのまま再現してください。[インテグレーション](../../../docs/user-guide/integrations.md)は、GitHub、GitLab、その他の webhook ソースから自動化を起動する別の機能です。
...
![Semaphore の公開デモリポジトリを入力したリポジトリフォーム](../../../static/assets/getting-started/repository-settings.jpg)

リポジトリが一覧に表示されます。Semaphore は、Repository レコードを作成したときではなく、タスク開始時に実行ホスト上でクローンまたは更新します。スクリーンショットはこのガイドのデモ値を示しています。

![プロジェクトのリポジトリ一覧に表示された接続済み Demo リポジトリ](../../../static/assets/getting-started/connected-repository.jpg)

非公開リポジトリでは、`None` の代わりに適切な Key Store の認証情報を選択します。ローカルパス、HTTPS、SSH、ブランチ、認証情報、依存関係ファイルについては[リポジトリ](../../../docs/user-guide/repositories.md)を参照してください。
...
![管理対象ホストのアカウント用 New SSH Key フォーム](../../../static/assets/getting-started/add-managed-host-ssh-key.jpg)

スクリーンショットはプレースホルダーであり、有効なシークレットではありません。秘密キーをドキュメント、スクリーンショット、タスク引数、バージョン管理に公開しないでください。

Semaphore はシークレットをローカルに保存するほか、[HashiCorp Vault](../../../docs/user-guide/key-store/hashicorp-vault.md) や [Devolutions Server](../../../docs/user-guide/key-store/devolutions-server.md) などの外部シークレットストレージと連携できます。対応するすべての認証情報の種類と保存方法は[キーストア](../../../docs/user-guide/key-store.md)を参照してください。
...
![デモリポジトリの値で設定した Ansible のファイル型インベントリ](../../../static/assets/getting-started/ansible-inventory-settings.jpg)
...
静的、ファイル型、動的インベントリについては[インベントリ](../../../docs/user-guide/inventory.md)を参照してください。
...
![テーブルエディターで設定した変数グループ](../../../static/assets/getting-started/variable-group-table.jpg)

優先順位とシークレット保存方法は[変数グループ](../../../docs/user-guide/environment.md)を参照してください。
...
デモは Git サブモジュールからそのロールをダウンロードし、the demo's configured public target へ ICMP リクエストを 1 回送信します。そのため実行ホストには GitHub へのアクセスと外向き ICMP が必要です。ICMP がブロックされる場合は、ローカルの `get-started.yml` の例を使ってください。

![接続した GitHub リポジトリ内の ping.yml](../../../static/assets/getting-started/demo-playbook-github.jpg)
...
![リポジトリ、インベントリ、変数グループを設定した Ansible タスクテンプレート](../../../static/assets/getting-started/ansible-task-template-settings.jpg)

<details>
<summary>この手順を動画で見る</summary>

![Ansible を有効にして最初の Ansible タスクテンプレートを作成](../../../static/assets/getting-started/create-ansible-template.gif)
...
すべてのフィールドと実行オプションは [Ansible テンプレート](../../../docs/user-guide/apps/ansible.md)と[タスクテンプレート](../../../docs/user-guide/task-templates/README.md)を参照してください。
...
![追加オプションを設定していない Ansible playbook の New Task ダイアログ](../../../static/assets/getting-started/run-ansible-task-clean.jpg)
...
![ping の出力と PLAY RECAP を含む成功した Ansible タスクのログ](../../../static/assets/getting-started/ansible-task-log-variable-group.jpg)
...
![テンプレート、コミット、時間情報を表示したタスク詳細](../../../static/assets/getting-started/ansible-task-details.jpg)

![OK と Not OK のホスト数を表示したタスクサマリー](../../../static/assets/getting-started/ansible-task-summary.jpg)
...
![Ansible タスクを実行し、ログと詳細を確認](../../../static/assets/getting-started/run-and-inspect-task.gif)

</details>

### 過去の実行を探す

タスクウィンドウを閉じると、テンプレートの **Tasks** タブに戻ります。各実行には固有のタスク番号、ステータス、ユーザー、開始時刻、所要時間、保存されたログがあります。**Dashboard → History** にはプロジェクトの全テンプレートの実行が表示されます。詳しくは[タスク](../../../docs/user-guide/tasks.md)と[プロジェクト履歴](../../../docs/user-guide/projects/history.md)を参照してください。

![成功したタスク実行を表示した Ansible テンプレートの履歴](../../../static/assets/getting-started/ansible-template-history.jpg)
...
![例のタスクを毎日 03:00 に実行する cron スケジュール](../../../static/assets/getting-started/create-cron-schedule.jpg)

Semaphore は保存前に設定済みのタイムゾーンと次回実行時刻を表示します。スケジュール実行はテンプレートと同じリポジトリ、インベントリ、変数グループ、実行設定を使います。テンプレートがプロンプトを公開している場合、スケジュールで値を指定できます。cron 構文、タイムゾーン設定、単発実行、スケジュールのパラメーターについては[スケジュール](../../../docs/user-guide/schedules.md)を参照してください。
...
- リポジトリに認証が必要な場合は、[キーストア](../../../docs/user-guide/key-store.md)に適切な非公開の認証情報を追加します。
- 複数のテンプレートに成功、失敗、承認、メモの順序付き経路が必要な場合は、**Workflow** を作成します。
- GitHub、GitLab などから認証付き webhook で起動するには、[インテグレーション](../../../docs/user-guide/integrations.md)を使います。
- リソースの管理やテンプレートの起動をプログラムから行うには、[API](../../../docs/reference/api.md) を使います。
- 別のネットワーク、OS、セキュリティ境界で実行する必要がある場合は、[リモート runner](../../../docs/admin-guide/runners.md) を追加します。

本番環境では Semaphore を HTTPS 経由で公開し、データベースとアクセスキー暗号化用シークレットを一緒にバックアップし、認証を一元化してください。[セキュリティ](../../../docs/admin-guide/security.md)、[ログ](../../../docs/admin-guide/logs.md)、[アップグレード](../../../docs/admin-guide/upgrading.md)も確認してください。

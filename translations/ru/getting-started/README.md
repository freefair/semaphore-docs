...
Для пошаговой настройки используйте официальную [страницу установки Semaphore](../../../docs/admin-guide/configuration/config-file.md), чтобы выбрать релиз, подготовить конфигурацию и получить соответствующие команды скачивания или запуска.
...
| --- | --- | --- |
| **Системный пакет** | Поддерживаемый сервер Linux | [Установка через менеджер пакетов](../../../docs/admin-guide/installation/package-manager.md) |
| **Docker Compose** | Быстрая изолированная установка или хост контейнеров | [Установка в Docker](../../../docs/admin-guide/installation/docker.md) |
| **Архив с исполняемым файлом** | macOS, Windows, FreeBSD или Linux без подходящего пакета | [Установка из исполняемого файла](../../../docs/admin-guide/installation/binary-file.md) |
| **Kubernetes с Helm** | Существующий кластер Kubernetes | [Установка в Kubernetes](../../../docs/admin-guide/installation/k8s.md) |
...
>
> Перед использованием Semaphore в production изучите разделы [Конфигурация](../../../docs/admin-guide/configuration.md), [Безопасность](../../../docs/admin-guide/security.md), [Раннеры](../../../docs/admin-guide/runners.md), [Высокая доступность](../../../docs/admin-guide/ha.md) и [Обновление](../../../docs/admin-guide/upgrading.md).
...
![Экран входа в Semaphore](../../../static/assets/getting-started/sign-in.jpg)

Для первоначальной настройки используйте учётную запись администратора: она позволяет создавать проекты и пользователей. Обычные пользователи входят на той же странице после того, как администратор создаст их учётные записи и предоставит доступ к проекту. См. [Управление пользователями](../../../docs/user-guide/admin/users.md).
...
![Пустая форма New Project со всеми доступными полями](../../../static/assets/getting-started/new-project-empty.jpg)

В новом проекте доступны разделы **Task Templates**, **Workflows**, **Schedule**, **Inventory**, **Variable Groups**, **Key Store** и **Repositories**. Настройки проекта, доступ команды, активность и история описаны в разделе [Проекты](../../../docs/user-guide/projects.md).

<details>
<summary>Посмотрите этот шаг</summary>

![Создание первого проекта в пустом экземпляре Semaphore](../../../static/assets/getting-started/create-first-project.gif)
...
![Интерфейс пустого проекта Semaphore до добавления ресурсов и задач](../../../static/assets/getting-started/after-sign-in.jpg)
...
На схеме показано, как эти ресурсы участвуют в запуске:
  ...
  ![Как ресурсы и триггеры Semaphore приводят к запуску задачи](../../../static/assets/getting-started/core-concepts.svg)
...
| Понятие | Назначение |
| --- | --- |
| [**Project**](../../../docs/user-guide/projects.md) | Изолированное рабочее пространство с ресурсами автоматизации, правами доступа и историей запусков. |
| [**Repository**](../../../docs/user-guide/repositories.md) | Указывает ветку или тег Git с файлами автоматизации, используемыми задачей. |
| [**Key Store**](../../../docs/user-guide/key-store.md) | Хранит переиспользуемые SSH-ключи, учётные данные, токены и пароли Ansible Vault вне Git и входных параметров задач. |
| [**Inventory**](../../../docs/user-guide/inventory.md) | Указывает Ansible управляемые хосты и группы, а также необходимые учётные данные. |
| [**Variable Group**](../../../docs/user-guide/environment.md) | Хранит переиспользуемые переменные Ansible, переменные окружения и секреты для одного или нескольких шаблонов. |
| [**Task Template**](../../../docs/user-guide/task-templates/README.md) | Сохраняет параметры запуска: тип автоматизации, файл, репозиторий, inventory, переменные, запрашиваемые параметры и настройки выполнения. |
| [**Task (task run)**](../../../docs/user-guide/tasks.md) | Одно выполнение со своими входными данными, статусом, временными метками, журналом, сведениями и результатом. |
...
![Форма репозитория с данными публичного демо Semaphore](../../../static/assets/getting-started/repository-settings.jpg)
...
![Подключённый репозиторий Demo в списке репозиториев проекта](../../../static/assets/getting-started/connected-repository.jpg)

Для приватного репозитория вместо `None` выберите подходящие учётные данные из Key Store. Локальные пути, HTTPS, SSH, ветки, учётные данные и файлы зависимостей описаны в разделе [Репозитории](../../../docs/user-guide/repositories.md).
...
![Форма New SSH Key для учётной записи на управляемых хостах](../../../static/assets/getting-started/add-managed-host-ssh-key.jpg)
...
Semaphore может хранить секреты локально или использовать внешние хранилища, например [HashiCorp Vault](../../../docs/user-guide/key-store/hashicorp-vault.md) и [Devolutions Server](../../../docs/user-guide/key-store/devolutions-server.md). Все поддерживаемые типы учётных данных и варианты хранения описаны в разделе [Хранилище ключей](../../../docs/user-guide/key-store.md).
...
![Файловый inventory Ansible с настройками демо-репозитория](../../../static/assets/getting-started/ansible-inventory-settings.jpg)
...
Статические, файловые и динамические inventory описаны в разделе [Inventory](../../../docs/user-guide/inventory.md).
...
![Группа переменных, настроенная в табличном редакторе](../../../static/assets/getting-started/variable-group-table.jpg)

Правила приоритета и варианты хранения секретов описаны в разделе [Группы переменных](../../../docs/user-guide/environment.md).
...
Демо загружает эту роль из подмодуля Git и отправляет один ICMP-запрос к the demo's configured public target, поэтому узлу выполнения нужны доступ к GitHub и исходящий ICMP. Если ICMP заблокирован, используйте локальный пример `get-started.yml`.

![ping.yml в подключённом репозитории GitHub](../../../static/assets/getting-started/demo-playbook-github.jpg)
...
![Шаблон задачи Ansible с репозиторием, inventory и группой переменных](../../../static/assets/getting-started/ansible-task-template-settings.jpg)

<details>
<summary>Посмотрите этот шаг</summary>

![Включение Ansible и создание первого шаблона задачи Ansible](../../../static/assets/getting-started/create-ansible-template.gif)
...
Все поля и настройки выполнения описаны в разделах [Шаблоны Ansible](../../../docs/user-guide/apps/ansible.md) и [Шаблоны задач](../../../docs/user-guide/task-templates/README.md).
...
![Диалог New Task для playbook Ansible без дополнительных настроек](../../../static/assets/getting-started/run-ansible-task-clean.jpg)
...
![Журнал успешной задачи Ansible с выводом ping и PLAY RECAP](../../../static/assets/getting-started/ansible-task-log-variable-group.jpg)
...
![Вкладка Details со сведениями о шаблоне, коммите и времени](../../../static/assets/getting-started/ansible-task-details.jpg)

![Сводка задачи с количеством хостов OK и Not OK](../../../static/assets/getting-started/ansible-task-summary.jpg)
...
![Запуск задачи Ansible и просмотр её журнала и подробностей](../../../static/assets/getting-started/run-and-inspect-task.gif)

</details>

### Найдите предыдущие запуски

Закройте окно задачи, чтобы вернуться на вкладку **Tasks** шаблона. Каждый запуск имеет свой номер задачи, статус, пользователя, время начала, длительность и сохранённый журнал. **Dashboard → History** показывает запуски всех шаблонов проекта. Подробнее см. [Задачи](../../../docs/user-guide/tasks.md) и [История проекта](../../../docs/user-guide/projects/history.md).

![История шаблона Ansible с успешными запусками задач](../../../static/assets/getting-started/ansible-template-history.jpg)
...
- Добавьте подходящие приватные учётные данные в [Хранилище ключей](../../../docs/user-guide/key-store.md), если репозиторий требует аутентификации.
- Создайте **Workflow**, если нескольким шаблонам нужны упорядоченные переходы для успеха, ошибки, согласования или заметок.
- Используйте [Интеграции](../../../docs/user-guide/integrations.md) для аутентифицированных webhook-триггеров из GitHub, GitLab и других систем.
- Используйте [API](../../../docs/reference/api.md) для программного управления ресурсами и запуска шаблонов.
- Добавьте [удалённый раннер](../../../docs/admin-guide/runners.md), если выполнение требуется в другой сети, операционной системе или зоне безопасности.

Для production разместите Semaphore за HTTPS, создавайте резервные копии базы данных вместе с секретом шифрования ключей доступа, настройте централизованную аутентификацию и изучите разделы [Безопасность](../../../docs/admin-guide/security.md), [Журналы](../../../docs/admin-guide/logs.md) и [Обновление](../../../docs/admin-guide/upgrading.md).

...
Pour une configuration guidée, utilisez la [page officielle d’installation de Semaphore](../../../docs/admin-guide/configuration/config-file.md) afin de sélectionner la version, générer la configuration et obtenir les commandes de téléchargement ou d’exécution correspondantes.
...
| --- | --- | --- |
| **Paquet natif** | Un serveur Linux pris en charge | [Installation par gestionnaire de paquets](../../../docs/admin-guide/installation/package-manager.md) |
| **Docker Compose** | Une installation isolée rapide ou un hôte de conteneurs | [Installation Docker](../../../docs/admin-guide/installation/docker.md) |
| **Archive binaire** | macOS, Windows, FreeBSD ou Linux sans paquet adapté | [Installation du binaire](../../../docs/admin-guide/installation/binary-file.md) |
| **Kubernetes avec Helm** | Un cluster Kubernetes existant | [Installation Kubernetes](../../../docs/admin-guide/installation/k8s.md) |
...
>
> Avant d’utiliser Semaphore en production, consultez [Configuration](../../../docs/admin-guide/configuration.md), [Sécurité](../../../docs/admin-guide/security.md), [Runners](../../../docs/admin-guide/runners.md), [Haute disponibilité](../../../docs/admin-guide/ha.md) et [Mise à niveau](../../../docs/admin-guide/upgrading.md).
...
![Écran de connexion de Semaphore](../../../static/assets/getting-started/sign-in.jpg)

Utilisez le compte administrateur pour la configuration initiale, car il peut créer des projets et des utilisateurs. Les utilisateurs ordinaires se connectent sur la même page après qu’un administrateur a créé leur compte et accordé l’accès au projet. Consultez [Gestion des utilisateurs](../../../docs/user-guide/admin/users.md).
...
![Formulaire New Project vide avec tous les champs disponibles](../../../static/assets/getting-started/new-project-empty.jpg)

Le nouveau projet comporte les sections **Task Templates**, **Workflows**, **Schedule**, **Inventory**, **Variable Groups**, **Key Store** et **Repositories**. Consultez [Projets](../../../docs/user-guide/projects.md) pour les paramètres du projet, l’accès de l’équipe, l’activité et l’historique.

<details>
<summary>Voir cette étape</summary>

![Création du premier projet dans une instance Semaphore vide](../../../static/assets/getting-started/create-first-project.gif)
...
![Interface d’un projet Semaphore vide avant l’ajout de ressources et de tâches](../../../static/assets/getting-started/after-sign-in.jpg)
...
Le schéma montre comment ces ressources produisent une exécution :
  ...
  ![Comment les ressources et déclencheurs Semaphore produisent une exécution de tâche](../../../static/assets/getting-started/core-concepts.svg)
...
| Concept | Fonction |
| --- | --- |
| [**Project**](../../../docs/user-guide/projects.md) | Un espace de travail isolé contenant les ressources d’automatisation, les autorisations et l’historique des exécutions. |
| [**Repository**](../../../docs/user-guide/repositories.md) | Désigne la branche ou le tag Git contenant les fichiers d’automatisation utilisés par une tâche. |
| [**Key Store**](../../../docs/user-guide/key-store.md) | Stocke les clés SSH, identifiants, jetons et mots de passe Ansible Vault réutilisables hors de Git et des entrées de tâches. |
| [**Inventory**](../../../docs/user-guide/inventory.md) | Indique à Ansible les hôtes et groupes à gérer ainsi que les identifiants à utiliser. |
| [**Variable Group**](../../../docs/user-guide/environment.md) | Stocke les variables Ansible, variables d’environnement et secrets réutilisables pour un ou plusieurs modèles. |
| [**Task Template**](../../../docs/user-guide/task-templates/README.md) | Enregistre les éléments à exécuter : type d’automatisation, fichier, dépôt, inventaire, variables, paramètres demandés et options d’exécution. |
| [**Task (task run)**](../../../docs/user-guide/tasks.md) | Une exécution avec ses propres entrées, état, horodatages, journal, détails et résultat. |
...
| [**Schedule**](../../../docs/user-guide/schedules.md) | Démarre un modèle de tâche ou un workflow une fois ou régulièrement selon une expression cron. |
| [**Runner**](../../../docs/admin-guide/runners.md) | Exécute les tâches en attente hors du serveur principal Semaphore, par exemple dans un autre réseau ou périmètre de sécurité. |

## 5. Connecter le dépôt

Un Repository relie Semaphore à l’automatisation stockée dans Git ; Semaphore ne stocke pas le playbook lui-même. Connectez votre dépôt ou utilisez les valeurs de démonstration publique ci-dessous pour reproduire exactement l’exemple. Les [Intégrations](../../../docs/user-guide/integrations.md) sont une fonction distincte permettant de lancer l’automatisation depuis GitHub, GitLab ou d’autres sources de webhooks.
...
![Formulaire de dépôt renseigné avec le dépôt public de démonstration Semaphore](../../../static/assets/getting-started/repository-settings.jpg)
...
![Dépôt Demo connecté dans la liste des dépôts du projet](../../../static/assets/getting-started/connected-repository.jpg)

Pour un dépôt privé, sélectionnez des identifiants adaptés dans le Key Store au lieu de `None`. Consultez [Dépôts](../../../docs/user-guide/repositories.md) pour les chemins locaux, HTTPS, SSH, branches, identifiants et fichiers de dépendances.
...
![Formulaire New SSH Key pour le compte utilisé sur les hôtes gérés](../../../static/assets/getting-started/add-managed-host-ssh-key.jpg)
...
Semaphore peut stocker les secrets localement ou intégrer des gestionnaires externes tels que [HashiCorp Vault](../../../docs/user-guide/key-store/hashicorp-vault.md) et [Devolutions Server](../../../docs/user-guide/key-store/devolutions-server.md). Consultez [Magasin de clés](../../../docs/user-guide/key-store.md) pour tous les types d’identifiants et options de stockage pris en charge.
...
![Inventaire Ansible de type fichier configuré avec les valeurs du dépôt de démonstration](../../../static/assets/getting-started/ansible-inventory-settings.jpg)
...
Consultez [Inventaire](../../../docs/user-guide/inventory.md) pour les inventaires statiques, basés sur des fichiers et dynamiques.
...
![Groupe de variables configuré dans l’éditeur de tableau](../../../static/assets/getting-started/variable-group-table.jpg)

Consultez [Groupes de variables](../../../docs/user-guide/environment.md) pour les règles de priorité et les options de stockage des secrets.
...
La démonstration télécharge ce rôle depuis un sous-module Git et envoie une requête ICMP à the demo's configured public target. L’hôte d’exécution doit donc accéder à GitHub et autoriser l’ICMP sortant. Si l’ICMP est bloqué, utilisez l’exemple local `get-started.yml`.

![ping.yml dans le dépôt GitHub connecté](../../../static/assets/getting-started/demo-playbook-github.jpg)
...
![Modèle de tâche Ansible avec dépôt, inventaire et groupe de variables](../../../static/assets/getting-started/ansible-task-template-settings.jpg)

<details>
<summary>Voir cette étape</summary>

![Activation d’Ansible et création du premier modèle de tâche Ansible](../../../static/assets/getting-started/create-ansible-template.gif)
...
Consultez [Modèles Ansible](../../../docs/user-guide/apps/ansible.md) et [Modèles de tâches](../../../docs/user-guide/task-templates/README.md) pour tous les champs et options d’exécution.
...
![Boîte de dialogue New Task sans options supplémentaires pour le playbook Ansible](../../../static/assets/getting-started/run-ansible-task-clean.jpg)
...
![Journal d’une tâche Ansible réussie avec sortie ping et PLAY RECAP](../../../static/assets/getting-started/ansible-task-log-variable-group.jpg)
...
![Détails de la tâche avec modèle, commit et informations temporelles](../../../static/assets/getting-started/ansible-task-details.jpg)

![Résumé de la tâche avec le nombre d’hôtes OK et Not OK](../../../static/assets/getting-started/ansible-task-summary.jpg)
...
![Exécution de la tâche Ansible et examen du journal et des détails](../../../static/assets/getting-started/run-and-inspect-task.gif)

</details>

### Retrouver les exécutions précédentes

Fermez la fenêtre de tâche pour revenir à l’onglet **Tasks** du modèle. Chaque exécution possède son numéro de tâche, son état, son utilisateur, son heure de début, sa durée et son journal conservé. **Dashboard → History** affiche les exécutions de tous les modèles du projet. Consultez [Tâches](../../../docs/user-guide/tasks.md) et [Historique du projet](../../../docs/user-guide/projects/history.md) pour en savoir plus.

![Historique d’un modèle Ansible avec des exécutions réussies](../../../static/assets/getting-started/ansible-template-history.jpg)
...
![Planification cron pour exécuter la tâche d’exemple chaque jour à 03:00](../../../static/assets/getting-started/create-cron-schedule.jpg)

Semaphore affiche le fuseau horaire configuré et calcule la prochaine exécution avant l’enregistrement. Une exécution planifiée utilise les mêmes dépôt, inventaire, groupes de variables et paramètres d’exécution que le modèle. Si le modèle expose des paramètres à renseigner, la planification peut fournir leurs valeurs. Consultez [Planifications](../../../docs/user-guide/schedules.md) pour la syntaxe cron, les fuseaux horaires, les exécutions uniques et les paramètres planifiés.
...
- Ajoutez les identifiants privés adaptés dans le [Magasin de clés](../../../docs/user-guide/key-store.md) si votre dépôt exige une authentification.
...
- Utilisez les [Intégrations](../../../docs/user-guide/integrations.md) pour les déclencheurs webhook authentifiés depuis GitHub, GitLab ou d’autres systèmes.
- Utilisez l’[API](../../../docs/reference/api.md) pour gérer les ressources et démarrer les modèles par programmation.
- Ajoutez un [runner distant](../../../docs/admin-guide/runners.md) si l’exécution doit se faire dans un autre réseau, système d’exploitation ou périmètre de sécurité.

Pour la production, exposez Semaphore via HTTPS, sauvegardez ensemble la base de données et le secret de chiffrement des clés d’accès, configurez une authentification centralisée et consultez [Sécurité](../../../docs/admin-guide/security.md), [Journaux](../../../docs/admin-guide/logs.md) et [Mise à niveau](../../../docs/admin-guide/upgrading.md).

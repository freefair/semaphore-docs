...
Per una configurazione guidata, usa la [pagina ufficiale di installazione di Semaphore](../../../docs/admin-guide/configuration/config-file.md) per scegliere la versione, generare la configurazione e ottenere i comandi di download o avvio corrispondenti.
...
| --- | --- | --- |
| **Pacchetto nativo** | Un server Linux supportato | [Installazione con gestore di pacchetti](../../../docs/admin-guide/installation/package-manager.md) |
| **Docker Compose** | Una configurazione isolata rapida o un host di container | [Installazione Docker](../../../docs/admin-guide/installation/docker.md) |
| **Archivio binario** | macOS, Windows, FreeBSD o Linux senza un pacchetto adatto | [Installazione del binario](../../../docs/admin-guide/installation/binary-file.md) |
| **Kubernetes con Helm** | Un cluster Kubernetes esistente | [Installazione Kubernetes](../../../docs/admin-guide/installation/k8s.md) |
...
>
> Prima di usare Semaphore in produzione, consulta [Configurazione](../../../docs/admin-guide/configuration.md), [Sicurezza](../../../docs/admin-guide/security.md), [Runner](../../../docs/admin-guide/runners.md), [Alta disponibilità](../../../docs/admin-guide/ha.md) e [Aggiornamento](../../../docs/admin-guide/upgrading.md).
...
![Schermata di accesso di Semaphore](../../../static/assets/getting-started/sign-in.jpg)

Usa l’account amministratore per la configurazione iniziale, perché può creare progetti e utenti. Gli utenti normali accedono dalla stessa pagina dopo che un amministratore ha creato i loro account e concesso l’accesso al progetto. Consulta [Gestione utenti](../../../docs/user-guide/admin/users.md).
...
![Modulo New Project vuoto con tutti i campi disponibili](../../../static/assets/getting-started/new-project-empty.jpg)

Il nuovo progetto contiene le sezioni **Task Templates**, **Workflows**, **Schedule**, **Inventory**, **Variable Groups**, **Key Store** e **Repositories**. Consulta [Progetti](../../../docs/user-guide/projects.md) per impostazioni, accesso del team, attività e cronologia.

<details>
<summary>Guarda questo passaggio</summary>

![Creazione del primo progetto in un’istanza Semaphore vuota](../../../static/assets/getting-started/create-first-project.gif)
...
![Interfaccia di un progetto Semaphore vuoto prima dell’aggiunta di risorse e attività](../../../static/assets/getting-started/after-sign-in.jpg)
...
Il diagramma mostra come queste risorse producono un’esecuzione:
  ...
  ![Come risorse e trigger di Semaphore generano l’esecuzione di un’attività](../../../static/assets/getting-started/core-concepts.svg)
...
| Concetto | Funzione |
| --- | --- |
| [**Project**](../../../docs/user-guide/projects.md) | Uno spazio di lavoro isolato con risorse di automazione, autorizzazioni e cronologia delle esecuzioni. |
| [**Repository**](../../../docs/user-guide/repositories.md) | Indica il branch o tag Git contenente i file di automazione usati da un’attività. |
| [**Key Store**](../../../docs/user-guide/key-store.md) | Conserva chiavi SSH, credenziali, token e password Ansible Vault riutilizzabili fuori da Git e dagli input delle attività. |
| [**Inventory**](../../../docs/user-guide/inventory.md) | Indica ad Ansible quali host e gruppi gestire e quali credenziali usare. |
| [**Variable Group**](../../../docs/user-guide/environment.md) | Conserva variabili Ansible, variabili d’ambiente e segreti riutilizzabili per uno o più modelli. |
| [**Task Template**](../../../docs/user-guide/task-templates/README.md) | Salva cosa eseguire: tipo di automazione, file, repository, inventario, variabili, parametri richiesti e opzioni di esecuzione. |
| [**Task (task run)**](../../../docs/user-guide/tasks.md) | Una singola esecuzione con input, stato, timestamp, log, dettagli e risultato propri. |
| **Workflow** | Collega modelli di attività in un percorso a più passi con rami di successo, errore, approvazione e note. |
| [**Schedule**](../../../docs/user-guide/schedules.md) | Avvia un modello di attività o workflow una volta o ripetutamente tramite un’espressione cron. |
| [**Runner**](../../../docs/admin-guide/runners.md) | Esegue le attività in coda fuori dal server Semaphore principale, ad esempio in un’altra rete o zona di sicurezza. |

## 5. Collega il repository

Un Repository collega Semaphore all’automazione in Git; Semaphore non memorizza il playbook stesso. Collega il tuo repository oppure usa i valori della demo pubblica riportati sotto per riprodurre esattamente l’esempio. Le [Integrazioni](../../../docs/user-guide/integrations.md) sono una funzione separata per avviare automazioni da GitHub, GitLab o altre sorgenti webhook.
...
![Modulo repository compilato con il repository pubblico della demo Semaphore](../../../static/assets/getting-started/repository-settings.jpg)
...
![Repository Demo collegato nell’elenco dei repository del progetto](../../../static/assets/getting-started/connected-repository.jpg)

Per un repository privato, scegli credenziali adeguate dal Key Store al posto di `None`. Consulta [Repository](../../../docs/user-guide/repositories.md) per percorsi locali, HTTPS, SSH, branch, credenziali e file di dipendenze.
...
![Modulo New SSH Key per l’account usato sugli host gestiti](../../../static/assets/getting-started/add-managed-host-ssh-key.jpg)
...
Semaphore può conservare i segreti localmente o integrare archivi esterni come [HashiCorp Vault](../../../docs/user-guide/key-store/hashicorp-vault.md) e [Devolutions Server](../../../docs/user-guide/key-store/devolutions-server.md). Consulta [Archivio chiavi](../../../docs/user-guide/key-store.md) per tutti i tipi di credenziali e le opzioni di archiviazione supportati.
...
![Inventario Ansible su file configurato con i valori del repository demo](../../../static/assets/getting-started/ansible-inventory-settings.jpg)
...
Consulta [Inventario](../../../docs/user-guide/inventory.md) per inventari statici, basati su file e dinamici.
...
![Gruppo di variabili configurato nell’editor a tabella](../../../static/assets/getting-started/variable-group-table.jpg)

Consulta [Gruppi di variabili](../../../docs/user-guide/environment.md) per le regole di precedenza e le opzioni di archiviazione dei segreti.
...
La demo scarica quel ruolo da un sottomodulo Git e invia una richiesta ICMP a the demo's configured public target, quindi l’host di esecuzione deve poter accedere a GitHub e inviare traffico ICMP. Se ICMP è bloccato, usa l’esempio locale `get-started.yml`.

![ping.yml nel repository GitHub collegato](../../../static/assets/getting-started/demo-playbook-github.jpg)
...
![Modello di attività Ansible con repository, inventario e gruppo di variabili](../../../static/assets/getting-started/ansible-task-template-settings.jpg)

<details>
<summary>Guarda questo passaggio</summary>

![Abilitazione di Ansible e creazione del primo modello di attività Ansible](../../../static/assets/getting-started/create-ansible-template.gif)
...
Consulta [Modelli Ansible](../../../docs/user-guide/apps/ansible.md) e [Modelli di attività](../../../docs/user-guide/task-templates/README.md) per tutti i campi e le opzioni di esecuzione.
...
![Finestra New Task senza opzioni aggiuntive per il playbook Ansible](../../../static/assets/getting-started/run-ansible-task-clean.jpg)
...
![Log di un’attività Ansible riuscita con output ping e PLAY RECAP](../../../static/assets/getting-started/ansible-task-log-variable-group.jpg)
...
![Dettagli dell’attività con modello, commit e informazioni temporali](../../../static/assets/getting-started/ansible-task-details.jpg)

![Riepilogo dell’attività con conteggi degli host OK e Not OK](../../../static/assets/getting-started/ansible-task-summary.jpg)
...
![Esecuzione dell’attività Ansible e verifica di log e dettagli](../../../static/assets/getting-started/run-and-inspect-task.gif)

</details>

### Trova le esecuzioni precedenti

Chiudi la finestra dell’attività per tornare alla scheda **Tasks** del modello. Ogni esecuzione ha numero attività, stato, utente, ora di inizio, durata e log conservato propri. **Dashboard → History** mostra le esecuzioni di tutti i modelli del progetto. Consulta [Attività](../../../docs/user-guide/tasks.md) e [Cronologia del progetto](../../../docs/user-guide/projects/history.md) per ulteriori dettagli.

![Cronologia di un modello Ansible con esecuzioni riuscite](../../../static/assets/getting-started/ansible-template-history.jpg)
...
![Pianificazione cron per eseguire l’attività di esempio ogni giorno alle 03:00](../../../static/assets/getting-started/create-cron-schedule.jpg)

Semaphore mostra il fuso orario configurato e calcola la prossima esecuzione prima del salvataggio. Un’esecuzione pianificata usa gli stessi repository, inventario, gruppi di variabili e impostazioni di esecuzione del modello. Se il modello espone parametri richiesti, la pianificazione può fornirne i valori. Consulta [Pianificazioni](../../../docs/user-guide/schedules.md) per sintassi cron, fusi orari, esecuzioni singole e parametri pianificati.
...
- Aggiungi credenziali private adeguate nell’[Archivio chiavi](../../../docs/user-guide/key-store.md) se il repository richiede autenticazione.
- Crea un **Workflow** quando più modelli richiedono percorsi ordinati di successo, errore, approvazione o note.
- Usa le [Integrazioni](../../../docs/user-guide/integrations.md) per trigger webhook autenticati da GitHub, GitLab o altri sistemi.
- Usa l’[API](../../../docs/reference/api.md) per gestire risorse e avviare modelli da codice.
- Aggiungi un [runner remoto](../../../docs/admin-guide/runners.md) se l’esecuzione deve avvenire in un’altra rete, sistema operativo o zona di sicurezza.

Per la produzione, esponi Semaphore tramite HTTPS, esegui il backup del database insieme al segreto di cifratura delle chiavi di accesso, configura l’autenticazione centralizzata e consulta [Sicurezza](../../../docs/admin-guide/security.md), [Log](../../../docs/admin-guide/logs.md) e [Aggiornamento](../../../docs/admin-guide/upgrading.md).

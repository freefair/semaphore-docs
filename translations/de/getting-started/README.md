...
Nutzen Sie für eine geführte Einrichtung die offizielle [Semaphore-Installationsseite](../../../docs/admin-guide/configuration/config-file.md), um die Version auszuwählen, die Konfiguration zu erstellen und die passenden Download- oder Startbefehle zu erhalten.
...
| --- | --- | --- |
| **Natives Paket** | Einen unterstützten Linux-Server | [Installation per Paketmanager](../../../docs/admin-guide/installation/package-manager.md) |
| **Docker Compose** | Eine schnelle isolierte Einrichtung oder einen Container-Host | [Docker-Installation](../../../docs/admin-guide/installation/docker.md) |
| **Binärarchiv** | macOS, Windows, FreeBSD oder Linux ohne passendes Paket | [Installation einer Binärdatei](../../../docs/admin-guide/installation/binary-file.md) |
| **Kubernetes mit Helm** | Einen vorhandenen Kubernetes-Cluster | [Kubernetes-Installation](../../../docs/admin-guide/installation/k8s.md) |
...
>
> Lesen Sie vor dem Produktivbetrieb von Semaphore die Seiten [Konfiguration](../../../docs/admin-guide/configuration.md), [Sicherheit](../../../docs/admin-guide/security.md), [Runner](../../../docs/admin-guide/runners.md), [Hochverfügbarkeit](../../../docs/admin-guide/ha.md) und [Upgrade](../../../docs/admin-guide/upgrading.md).
...
![Anmeldeseite von Semaphore](../../../static/assets/getting-started/sign-in.jpg)

Verwenden Sie für die Ersteinrichtung das Administratorkonto, da es Projekte und Benutzer erstellen kann. Normale Benutzer melden sich auf derselben Seite an, nachdem ein Administrator ihr Konto erstellt und Projektzugriff gewährt hat. Siehe [Benutzerverwaltung](../../../docs/user-guide/admin/users.md).
...
![Leeres Formular New Project mit allen verfügbaren Feldern](../../../static/assets/getting-started/new-project-empty.jpg)

Das neue Projekt enthält die Bereiche **Task Templates**, **Workflows**, **Schedule**, **Inventory**, **Variable Groups**, **Key Store** und **Repositories**. Unter [Projekte](../../../docs/user-guide/projects.md) finden Sie Informationen zu Projekteinstellungen, Teamzugriff, Aktivitäten und Verlauf.

<details>
<summary>Diesen Schritt ansehen</summary>

![Erstellen des ersten Projekts in einer leeren Semaphore-Instanz](../../../static/assets/getting-started/create-first-project.gif)
...
![Leere Semaphore-Projektoberfläche vor dem Hinzufügen von Ressourcen und Aufgaben](../../../static/assets/getting-started/after-sign-in.jpg)
...
Das Diagramm zeigt, wie diese Ressourcen zu einer Ausführung führen:
  ...
  ![Wie Semaphore-Ressourcen und Auslöser eine Aufgabenausführung erzeugen](../../../static/assets/getting-started/core-concepts.svg)
...
| Begriff | Funktion |
| --- | --- |
| [**Project**](../../../docs/user-guide/projects.md) | Ein isolierter Arbeitsbereich mit Automatisierungsressourcen, Berechtigungen und Ausführungsverlauf. |
| [**Repository**](../../../docs/user-guide/repositories.md) | Verweist auf den Git-Branch oder -Tag mit den Automatisierungsdateien einer Aufgabe. |
| [**Key Store**](../../../docs/user-guide/key-store.md) | Speichert wiederverwendbare SSH-Schlüssel, Zugangsdaten, Tokens und Ansible-Vault-Passwörter außerhalb von Git und Aufgabeneingaben. |
| [**Inventory**](../../../docs/user-guide/inventory.md) | Gibt Ansible die zu verwaltenden Hosts und Gruppen sowie die zu verwendenden Zugangsdaten vor. |
| [**Variable Group**](../../../docs/user-guide/environment.md) | Speichert wiederverwendbare Ansible-Variablen, Umgebungsvariablen und Geheimnisse für eine oder mehrere Vorlagen. |
| [**Task Template**](../../../docs/user-guide/task-templates/README.md) | Speichert die Ausführungsvorgaben: Automatisierungstyp, Datei, Repository, Inventory, Variablen, Eingabeaufforderungen und Ausführungsoptionen. |
| [**Task (task run)**](../../../docs/user-guide/tasks.md) | Eine einzelne Ausführung mit eigenen Eingaben, Status, Zeitstempeln, Protokoll, Details und Ergebnis. |
| **Workflow** | Verbindet Aufgabenvorlagen zu einem mehrstufigen Ablauf mit Zweigen für Erfolg, Fehler, Genehmigung und Notizen. |
| [**Schedule**](../../../docs/user-guide/schedules.md) | Startet eine Aufgabenvorlage oder einen Workflow einmalig oder wiederholt anhand eines Cron-Ausdrucks. |
| [**Runner**](../../../docs/admin-guide/runners.md) | Führt eingereihte Aufgaben außerhalb des Hauptservers von Semaphore aus, etwa in einem anderen Netzwerk oder Sicherheitsbereich. |

## 5. Das Repository verbinden

Ein Repository verbindet Semaphore mit Automatisierung in Git; Semaphore speichert das Playbook selbst nicht. Verbinden Sie Ihr eigenes Repository oder verwenden Sie die folgenden öffentlichen Demowerte, um das Beispiel exakt nachzuvollziehen. [Integrationen](../../../docs/user-guide/integrations.md) sind eine separate Funktion zum Starten von Automatisierung aus GitHub, GitLab oder anderen Webhook-Quellen.
...
![Repository-Formular mit dem öffentlichen Semaphore-Demorepository](../../../static/assets/getting-started/repository-settings.jpg)
...
![Verbundenes Demo-Repository in der Repository-Liste des Projekts](../../../static/assets/getting-started/connected-repository.jpg)

Wählen Sie für ein privates Repository passende Zugangsdaten aus dem Key Store statt `None`. Unter [Repositories](../../../docs/user-guide/repositories.md) finden Sie lokale Pfade, HTTPS, SSH, Branches, Zugangsdaten und Requirements-Dateien.
...
![Formular New SSH Key für das Konto auf den verwalteten Hosts](../../../static/assets/getting-started/add-managed-host-ssh-key.jpg)
...
Semaphore kann Geheimnisse lokal speichern oder externe Geheimnisspeicher wie [HashiCorp Vault](../../../docs/user-guide/key-store/hashicorp-vault.md) und [Devolutions Server](../../../docs/user-guide/key-store/devolutions-server.md) einbinden. Alle unterstützten Zugangsdatenarten und Speicheroptionen finden Sie unter [Key Store](../../../docs/user-guide/key-store.md).
...
![Ansible-Datei-Inventory mit den Werten des Demorepositorys](../../../static/assets/getting-started/ansible-inventory-settings.jpg)
...
Statische, dateibasierte und dynamische Inventories werden unter [Inventory](../../../docs/user-guide/inventory.md) beschrieben.
...
![Im Tabelleneditor konfigurierte Variablengruppe](../../../static/assets/getting-started/variable-group-table.jpg)

Vorrangregeln und Speicheroptionen für Geheimnisse finden Sie unter [Variablengruppen](../../../docs/user-guide/environment.md).
...
Das Demo lädt diese Rolle aus einem Git-Submodul herunter und sendet eine ICMP-Anfrage an the demo's configured public target. Der Ausführungshost benötigt daher GitHub-Zugriff und ausgehendes ICMP. Ist ICMP gesperrt, verwenden Sie stattdessen das lokale Beispiel `get-started.yml`.

![ping.yml im verbundenen GitHub-Repository](../../../static/assets/getting-started/demo-playbook-github.jpg)
...
![Ansible-Aufgabenvorlage mit Repository, Inventory und Variablengruppe](../../../static/assets/getting-started/ansible-task-template-settings.jpg)

<details>
<summary>Diesen Schritt ansehen</summary>

![Ansible aktivieren und die erste Ansible-Aufgabenvorlage erstellen](../../../static/assets/getting-started/create-ansible-template.gif)
...
Alle Felder und Ausführungsoptionen finden Sie unter [Ansible-Vorlagen](../../../docs/user-guide/apps/ansible.md) und [Aufgabenvorlagen](../../../docs/user-guide/task-templates/README.md).
...
![Unveränderter Dialog New Task für das Ansible-Playbook](../../../static/assets/getting-started/run-ansible-task-clean.jpg)
...
![Erfolgreiches Ansible-Aufgabenprotokoll mit Ping-Ausgabe und PLAY RECAP](../../../static/assets/getting-started/ansible-task-log-variable-group.jpg)
...
![Aufgabendetails mit Vorlage, Commit und Zeitangaben](../../../static/assets/getting-started/ansible-task-details.jpg)

![Aufgabenzusammenfassung mit Hostanzahlen für OK und Not OK](../../../static/assets/getting-started/ansible-task-summary.jpg)
...
![Ansible-Aufgabe ausführen und Protokoll sowie Details prüfen](../../../static/assets/getting-started/run-and-inspect-task.gif)

</details>

### Frühere Ausführungen finden

Schließen Sie das Aufgabenfenster, um zum Tab **Tasks** der Vorlage zurückzukehren. Jeder Lauf hat eine eigene Aufgabennummer, einen Status, Benutzer, Startzeitpunkt, eine Dauer und ein gespeichertes Protokoll. **Dashboard → History** zeigt Läufe aller Vorlagen im Projekt. Weitere Informationen finden Sie unter [Aufgaben](../../../docs/user-guide/tasks.md) und [Projektverlauf](../../../docs/user-guide/projects/history.md).

![Verlauf einer Ansible-Vorlage mit erfolgreichen Aufgabenausführungen](../../../static/assets/getting-started/ansible-template-history.jpg)
...
![Cron-Zeitplan für die tägliche Ausführung der Beispielaufgabe um 03:00 Uhr](../../../static/assets/getting-started/create-cron-schedule.jpg)

Semaphore zeigt die konfigurierte Zeitzone und berechnet den nächsten Lauf vor dem Speichern. Ein geplanter Lauf verwendet dasselbe Repository, Inventory, dieselben Variablengruppen und Ausführungseinstellungen wie die Vorlage. Wenn die Vorlage Eingabeaufforderungen anbietet, kann der Zeitplan dafür Werte angeben. Unter [Zeitpläne](../../../docs/user-guide/schedules.md) finden Sie Cron-Syntax, Zeitzonenkonfiguration, einmalige Läufe und geplante Parameter.
...
- Ergänzen Sie passende private Zugangsdaten im [Key Store](../../../docs/user-guide/key-store.md), wenn Ihr Repository Authentifizierung erfordert.
- Erstellen Sie einen **Workflow**, wenn mehrere Vorlagen geordnete Pfade für Erfolg, Fehler, Genehmigung oder Notizen benötigen.
- Verwenden Sie [Integrationen](../../../docs/user-guide/integrations.md) für authentifizierte Webhook-Auslöser aus GitHub, GitLab oder anderen Systemen.
- Verwenden Sie die [API](../../../docs/reference/api.md), um Ressourcen programmatisch zu verwalten und Vorlagen zu starten.
- Ergänzen Sie einen [Remote-Runner](../../../docs/admin-guide/runners.md), wenn die Ausführung in einem anderen Netzwerk, Betriebssystem oder Sicherheitsbereich erfolgen muss.

Stellen Sie Semaphore im Produktivbetrieb hinter HTTPS bereit, sichern Sie Datenbank und Geheimnis zur Verschlüsselung der Zugriffsschlüssel gemeinsam, richten Sie zentrale Authentifizierung ein und lesen Sie [Sicherheit](../../../docs/admin-guide/security.md), [Protokolle](../../../docs/admin-guide/logs.md) und [Upgrade](../../../docs/admin-guide/upgrading.md).

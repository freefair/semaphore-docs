...
- **Docker (pacotes adicionais)**: monte um `requirements.txt` em `/etc/semaphore/requirements.txt` no contêiner do servidor ou do runner. O Semaphore o instala no ambiente virtual Python incluído a cada inicialização do contêiner. Consulte [Instalando dependências Python adicionais](../../../../docs/admin-guide/installation/docker.md#installing-additional-python-dependencies).

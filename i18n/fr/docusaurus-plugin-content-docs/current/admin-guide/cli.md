...
La plupart des tâches d'administration disposent d'un groupe de commandes dédié :

| Groupe de commandes | Rôle |
|---------------------|------|
| [`semaphore users`](/admin-guide/cli/users) | Ajouter, modifier, supprimer et consulter des utilisateurs ; gérer les tokens d'API et le TOTP (2FA). |
| [`semaphore projects`](/admin-guide/cli/projects) | Exporter et importer des projets (sauvegardes). |
| [`semaphore vaults`](/admin-guide/cli/vaults) | Rechiffrer les secrets stockés et consulter l'utilisation des clés de chiffrement. |
| [`semaphore runner`](/admin-guide/cli/runners) | Fonctionner en mode runner et enregistrer/désenregistrer des runners. |
| [`semaphore migrate`](/admin-guide/cli/migrations) | Appliquer ou annuler des migrations de base de données. |
...
Exécute Semaphore en tant que runner de tâches. Consultez [Runners](/admin-guide/cli/runners) pour
...
[Migrations de base de données](/admin-guide/cli/migrations) pour appliquer ou annuler des

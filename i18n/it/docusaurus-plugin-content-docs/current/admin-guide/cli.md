...
La maggior parte delle attività amministrative dispone di un gruppo di comandi dedicato:

| Gruppo di comandi | Scopo |
|---------------|---------|
| [`semaphore users`](/admin-guide/cli/users) | Aggiungere, modificare, rimuovere e ispezionare utenti; gestire token API e TOTP (2FA). |
| [`semaphore projects`](/admin-guide/cli/projects) | Esportare e importare progetti (backup). |
| [`semaphore vaults`](/admin-guide/cli/vaults) | Ricifrare i segreti memorizzati e ispezionare l'uso delle chiavi di cifratura. |
| [`semaphore runner`](/admin-guide/cli/runners) | Eseguire in modalità runner e registrare/deregistrare i runner. |
| [`semaphore migrate`](/admin-guide/cli/migrations) | Applicare o annullare le migrazioni del database. |
...
Esegue Semaphore come runner di task. Consultare [Runner](/admin-guide/cli/runners) per
...
[Migrazioni del database](/admin-guide/cli/migrations) per applicare o annullare le migrazioni

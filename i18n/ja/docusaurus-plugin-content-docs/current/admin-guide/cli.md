...
ほとんどの管理タスクには専用のコマンドグループがあります:

| コマンドグループ | 用途 |
|---------------|---------|
| [`semaphore users`](/admin-guide/cli/users) | ユーザーの追加、変更、削除、参照。API トークンと TOTP (2FA) の管理。 |
| [`semaphore projects`](/admin-guide/cli/projects) | プロジェクトのエクスポートとインポート (バックアップ)。 |
| [`semaphore vaults`](/admin-guide/cli/vaults) | 保存されたシークレットの再暗号化と、暗号化キーの使用状況の確認。 |
| [`semaphore runner`](/admin-guide/cli/runners) | ランナーモードでの実行と、ランナーの登録/登録解除。 |
| [`semaphore migrate`](/admin-guide/cli/migrations) | データベースマイグレーションの適用またはロールバック。 |
...
`start`、`unregister`) は[ランナー](/admin-guide/cli/runners)を参照してください。
...
ついては[データベースマイグレーション](/admin-guide/cli/migrations)を参照してください。

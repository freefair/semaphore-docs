...
대부분의 관리 작업에는 전용 명령 그룹이 있습니다.

| 명령 그룹 | 용도 |
|---------------|---------|
| [`semaphore users`](/admin-guide/cli/users) | 사용자 추가, 변경, 삭제, 조회 및 API token과 TOTP(2FA) 관리. |
| [`semaphore projects`](/admin-guide/cli/projects) | 프로젝트 내보내기 및 가져오기(백업). |
| [`semaphore vaults`](/admin-guide/cli/vaults) | 저장된 비밀 정보 재암호화 및 암호화 키 사용 현황 확인. |
| [`semaphore runner`](/admin-guide/cli/runners) | runner 모드로 실행하고 runner를 등록/등록 해제. |
| [`semaphore migrate`](/admin-guide/cli/migrations) | 데이터베이스 마이그레이션 적용 또는 롤백. |
...
`start`, `unregister`)은 [Runner](/admin-guide/cli/runners)를 참고하십시오.
...
롤백하는 방법은 [데이터베이스 마이그레이션](/admin-guide/cli/migrations)을
참고하십시오.
